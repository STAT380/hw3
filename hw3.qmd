...
rnum_white <- nrow(df1)
rnum_red <- nrow(df2)
sp_squared <- ((rnum_white - 1) * var(df1$quality) + (rnum_red - 1) * var(df2$quality)) / (rnum_white + rnum_red - 2)

t1 <- diff_mean / sqrt(sp_squared * (1 / rnum_white + 1 / rnum_red))
...
# Insert your code here
```
t1 and t2 have the same value, which means manual calculation and t.test() function are both reliable.
t3 also has the same value means the type of wine is a significant predictor of quality of wine in this model.
...
summary_model_citric <- summary(model_citric)

print(summary_model_citric)
# Insert your code here
```
```{R}
model_sulfur <- lm(quality ~ total_sulfur_dioxide, data = df)

summary_model_sulfur <- summary(model_sulfur)

print(summary_model_sulfur)
# Insert your code here
```
Neither of citric_acid and total_sulfur_dioxide are the significant predictor of wine quality in previous question, but both of them are significant predictor in this model.
...
vif_df <- vif(full_model)

vif_df
```
The variables with vif value lower than 5 means these variable could provide unique information to this model.
...
backward_formula <- formula(stepwise)
...
scope_formula <- as.formula(paste("quality ~", paste(names(df)[!names(df) %in% "quality"], collapse = "+")))

stepwise_forward <- step(null_model, direction = "forward", scope = scope_formula)

forward_formula <- formula(stepwise_forward)
...
Stepwise selection and LASSO are good choice to intepret the model since they reduce the complexity of model, while ridge is more about prediction accuracy.
...
$2^10$ possible predictors. Since each predictor can either be include or not in a model.
...
comb_summaries <- bind_rows(summaries)
```
...
```{R}
summaries_formula <- bind_rows(summaries, .id = "model_id") %>%
  mutate(formula = formulas[as.integer(model_id)])

highest_r <- summaries_formula %>%
...
```{R}
rsq_formula <- highest_r$formula # Insert your code
```
...
```{R}
aic_formula <- lowest_aic # Insert your code
```
...
They are not the same. Backward/forward formula can find a balance between simplicity and predictive reliability, while LASSO/ridge formula reduce the complexity for large data.


* Which of these is more reliable? Why? 
AIC is more reliable since it penalizes model complexity.

* If we had a dataset with $10,000$ columns, which of these methods would you consider for your analyses? Why?
LASSO and ridge formula, since they have ability to reduce the complexity of big data.
